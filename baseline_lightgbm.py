...
print("Number of positive and negative return samples in training set:")
...
def lightgbm_classifier(x_train, y_train, x_test, n_splits=5, description="Initial Run"):
    print(f"Training LightGBM Classifier ({description})")
    ...
    for i, (train_date_idx, val_date_idx) in enumerate(kf.split(train_dates)):
        ...
        model = lgb.LGBMClassifier(n_estimators=100, max_depth=6, n_jobs=-1, verbose=-1)
        model.fit(X_t, y_t)
    ...
    results_logger.log_results("LightGBM_Baseline", scores, mean_acc, std_acc, description)
...
print("\n--- Initial Run ---")
test_predictions = lightgbm_classifier(x_train, y_train, x_test, description="Initial Run")
...
test_predictions = lightgbm_classifier(x_train, y_train, x_test, description="With Feature Engineering")


print("\n--- With Pair Trading Features ---")
...
test_predictions = lightgbm_classifier(x_train, y_train, x_test, description="With Pair Trading Features")

# Save submission
submission = pd.DataFrame({'ID': x_test.index, 'RET': test_predictions.astype(bool)})
submission.to_csv('submission_baseline_lightgbm.csv', index=False)
print("Saved submission_baseline_lightgbm.csv")



# │ --- Initial Run ---                                                                                                                                          │
# │ Training LightGBM Classifier                                                                                                                                 │                                                                                                                                   │
# │ Overall Accuracy: 0.5115 (+- 0.0029)
#   Overall Accuracy: 0.5132 (+- 0.0028)
#  Overall Accuracy: 0.5144 (+- 0.0108)                                                                                                                         │
# │                                                                                                                                                              │
# │ --- With Feature Engineering ---                                                                                                                             │
# │ Training LightGBM Classifier                                                                                                                                 │                                                                                                                                │
# │ Overall Accuracy: 0.5127 (+- 0.0033)   
#   Overall Accuracy: 0.5127 (+- 0.0031)
# Overall Accuracy: 0.5152 (+- 0.0098)                                                                                                                      │
# │                                                                                                                                                              │
# │ --- With Pair Trading Features ---                                                                                                                           │
# │ Training LightGBM Classifier                                                                                                                                 │                                                                                                                                   │
# │ Overall Accuracy: 0.5116 (+- 0.0065)
#   Overall Accuracy: 0.5142 (+- 0.0068)
# Overall Accuracy: 0.5133 (+- 0.0092)                                                                                                                  │
